...
\begin{description}
	...
	\item[5.] normalize the velocity perturbation 
		...
		Multiply the perturbation by $\epsilon_{u}$ and add it to
		the base state 
		$S^{steady,(0)}$:
                \begin{eqnarray}
	...
	\item[6.] normalize the temperature perturbation 
		so that the maximum value is one.
		Multiply the perturbation by $\epsilon_{T}$ and add it to
		the base state
		$S^{steady,(0)}$:
                \begin{eqnarray}
	...
	\item[7.] Make sure the input levelset  perturbation is
		...
		function.  In otherwords, $\delta\phi=0$ when 
		...
		\begin{eqnarray*}
			...
			|\delta\phi|
		\end{eqnarray*}
		Multiply $\delta\phi$ by $\sigma\Delta x/\beta$.  $\sigma=2$
		in our preliminary simulations.  Add the levelset perturbation
		to the base state $\phi^{steady,(0)}$:
                \begin{eqnarray}
                \phi^{perturb,(0)}=\phi^{steady,(0)}+
                   \frac{\sigma\Delta x}{\beta}(\delta \phi^{(k_{power})})
                \end{eqnarray}
        \item[8.] Solve,
        \begin{eqnarray}
	S^{perturb,n+1}=S^{perturb,n}+\Delta t \tilde{N}(S^{perturb,n}) 
           \label{perturb_discrete}
        \end{eqnarray}
        for $M$ time steps, with a fixed $\Delta t$.
        \item[9.] Update the perturbation prediction:
                \begin{eqnarray}
                ...
                \end{eqnarray}
        \item[10.] $k_{power}=k_{power}+1$
        \item[11.] Go back to step 5.
