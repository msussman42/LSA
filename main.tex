...
\title{A non-intrusive numerical method for determining the most
...
Instead of explicitly linearizing a discretized system of differential
equations and then
investigating the spectrum of the linearized equations, it is proposed
to do a ``matrix-free'' DMD method approach.  In the proposed
linear stability analysis approach, the largest (in magnitude)
eigenvalue of the linearized discretized equations is found.
The new 
DMD method approach determines the action of the linear operator, 
...
complex topology problems.  Examples are included for the Rayleigh capillary
instability problem in the axisymmetric $R$-$Z$ coordinate system.
%%and a Jet in Cross Flow problem. (3D debugging much more difficult than
%% R-Z debugging.)
...
The research question at hand is ``can one do a linear stability analysis
of complex multiphase flows?''  The classical approaches for these
kinds of problems become very complicated as the ``linearize then
discretize'' approach becomes unmanagible for complex liquid/gas interface
structure.   We refer to 
The following are some earlier works for the linear stability 
analysis of multiphase flows.   
In \cite{amini2014instability},
the classical approach of ``linearize then discretize'' was presented for
analyzing stability of an elliptic liquid jet in air.
In \cite{10.1063/1.5043589},
the classical approach of ``linearize then discretize'' was presented for
analyzing stability of a liquid jet in weak crossflowing air.
In \cite{kasmaiee2024linear},
the classical approach of ``linearize then discretize'' was presented for
analyzing stability of surface waves of a liquid jet injected in
...
In the liquid:
\begin{eqnarray}
	\rho^{L}\frac{Du^{L}}{Dt}=\nabla\cdot(2\mu D^{L}-p^{L}I)
\end{eqnarray}
\begin{eqnarray}
        \rho^{L}c_{v}^{L}\frac{DT^{L}}{Dt}=\nabla\cdot k\nabla T^{L}
\end{eqnarray}
...
In the gas:
\begin{eqnarray}
	\rho^{G}\frac{Du^{G}}{Dt}=\nabla\cdot(2\mu D^{G}-p^{L}I)
\end{eqnarray}
\begin{eqnarray}
        \rho^{G}c_{v}^{G}\frac{DT^{G}}{Dt}=\nabla\cdot k\nabla T^{G}
\end{eqnarray}
\begin{eqnarray}
	\nabla\cdot u^{G}=0
\end{eqnarray}
...
\kappa=\nabla\cdot n
\end{eqnarray}
$n$ is the interface normal, written in terms of the level set function $\phi$:
\begin{eqnarray}
	n=\frac{\nabla\phi}{|\nabla\phi|}
\end{eqnarray}
The liquid occupies the region where $\phi>0$ and the gas occupies the
region where $\phi<0$ and $\phi$ satisfies the following equation:
\begin{eqnarray}
	\phi_{t}+u \cdot\nabla\phi=0
\end{eqnarray}
...
\begin{eqnarray}
	...
	u \\
        T \end{array} \right)
\end{eqnarray}
After discretization of the multiphase flow equation, one can write:
\begin{eqnarray}
	S^{n+1}=S^{n}+\Delta t \tilde{N}(S^{n}) \label{discrete}
\end{eqnarray}
The proposed algorithm, ``discretize then differentiate,''
for linear stability analysis of two phase flows is
as follows:
\begin{description}
	\item[1.] compute (\ref{discrete}) until a desired ``steady state'' is
		reached.  We call the steady state $S^{steady,(0)}$.
                $\bar{Q}$ in \cite{RANJAN2020109076}.
	\item[2.] Using $S^{steady,(0)}$ as initial conditions, we solve 
		(\ref{discrete}) for $M$ time steps, with a fixed $\Delta t$.
		We call the resulting new state 
		$S_{alt}^{steady,(0)}$; this state can be also classified
		as the ``no perturbation state.''
		In other words,
		$S_{alt}^{steady,(0)}$ 
		is the state that one obtains if the
		applied perturbation is null.  
                $\bar{Q}+\Delta \bar{Q}$ in \cite{RANJAN2020109076}.
                In mathematical form,
                we define the overall 
                nonlinear operator encompassing the $M$ time steps as:
                \begin{eqnarray}
	           N^{(M)}\equiv (I+\Delta t \tilde{N})^{M}
                \end{eqnarray}
                $N^{(M)}$ is the ``M'' time step discrete solution operator.
		Then we have:
                \begin{eqnarray}
		S_{alt}^{steady,(0)}=N^{(M)}(S^{steady,(0)})
                \end{eqnarray}


        \item[3.] $k_{DMD}=0$
        \item[4.] Construct default perturbation(s):
                \begin{eqnarray}
                \delta u^{(k_{DMD})}
                \end{eqnarray}
                \begin{eqnarray}
                ...
                \delta \phi^{(k_{DMD})}
                \end{eqnarray}
                $Q^{\prime}$ in \cite{RANJAN2020109076}.

 
	\item[5.] (a) normalize the velocity perturbation 
		so that the maximum value is one.  The normalized velocity
		perturbation can be written as:
			...
			{\alpha_{u}^{(k_{DMD})}}
                \end{eqnarray}
		(b) Multiply the normalized velocity field by a representative
		perturbation factor:
                \begin{eqnarray}
			\delta u^{(k_{DMD})}=
			\epsilon_{u} \delta u^{(k_{DMD})}
                \end{eqnarray}

	\item[6.] (a) normalize the temperature perturbation 
		so that the maximum value is one.  The normalized temperature
		...
		{\alpha_{T}^{(k_{DMD})}}
                \end{eqnarray}
		(b) Multiply the normalized temperature field by a 
		representative perturbation factor:
                \begin{eqnarray}
                \delta T^{(k_{DMD})}=
		\frac{\delta T^{(k_{DMD})}}
		{\alpha_{T}^{(k_{DMD})}}
                \end{eqnarray}
	\item[7.] (a) Make sure the input levelset  perturbation is
		zero outside of a user prescribed narrow band thickness
		$R_{narrow}$ with respect to the unperturbed level set 
		function.  In otherwords, 
		$\delta\phi^{(k_{DMD})}=0$ when 
		$|\phi^{steady,(0)}|>R_{narrow}$.  Define the following
		interface normalization factor:
		\begin{eqnarray*}
			\beta=\max_{|\phi^{steady,(0)}|<\Delta x} 
			|\delta\phi^{(k_{DMD})}|
		\end{eqnarray*}
		Multiply $\delta\phi^{(k_{DMD})}$ by $1/\beta$; the 
		normalized
		level set perturbation is
                \begin{eqnarray}
		        \delta\phi^{(k_{DMD})}=
			\frac{\delta\phi^{(k_{DMD})}}
			{\beta^{(k_{DMD})}}
		\end{eqnarray}
		(b) Multiply the normalized levelset field by a 
		representative perturbation factor:
                \begin{eqnarray}
		        \delta\phi^{(k_{DMD})}=
			\sigma\Delta x \delta\phi^{(k_{DMD})}
		\end{eqnarray}

	\item[8.] Add the perturbations to the base 
		state $S^{steady,(0)}$ (velocity, temperature, and 
		level set function):
                \begin{eqnarray}
                u^{perturb,(0)}=u^{steady,(0)}+\delta u^{(k_{DMD})}
                \end{eqnarray}
                \begin{eqnarray}
		T^{perturb,(0)}=T^{steady,(0)}+\delta T^{(k_{DMD})}
                \end{eqnarray}
                \begin{eqnarray}
                \phi^{perturb,(0)}=\phi^{steady,(0)}+
			\delta \phi^{(k_{DMD})}
                \end{eqnarray}
		note: we can write the overall perturbation in the
		following form:
                \begin{eqnarray}
			\delta S^{(k_{DMD})}=\left(
			\begin{array}{c}
			...
			\end{array} \right)
                \end{eqnarray}
                \begin{eqnarray}
                S^{perturb,(0)}=S^{steady,(0)}+\delta S^{(k_{DMD})}
                \end{eqnarray}
                $\bar{Q}+Q^{\prime n}$ in \cite{RANJAN2020109076}
                ($n=k_{DMD}$).

        \item[9.] Solve,
        \begin{eqnarray}
		S^{perturb,(n+1)}=S^{perturb,(n)}+
		\Delta t \tilde{N}(S^{perturb,(n)}) 
           \label{perturb_discrete}
        \end{eqnarray}
        for $M$ time steps, with a fixed $\Delta t$.
        \item[10.] Update the perturbation prediction:
                \begin{eqnarray}
                \delta u^{(k_{DMD}+1)}=u^{perturb,M}-u_{alt}^{(0)}
                \end{eqnarray}
                \begin{eqnarray}
                \delta T^{(k_{DMD}+1)}=T^{perturb,M}-T_{alt}^{(0)}
                \end{eqnarray}
                \begin{eqnarray}
                \delta \phi^{(k_{DMD}+1)}=\phi^{perturb,M}-\phi_{alt}^{(0)}
                \end{eqnarray}
                \begin{eqnarray}
                \bar{Q}+Q^{\prime n}+\Delta\bar{Q}+\Delta Q^{\prime n}-
                (\bar{Q}+\Delta\bar{Q})\equiv Q^{\prime(n+1)}
                \end{eqnarray}
                in \cite{RANJAN2020109076}
        \item[11.] $k_{DMD}=k_{DMD}+1$
           ($n$ in \cite{RANJAN2020109076})
        \item[12.] Go back to step 8.

\end{description}
Remark: the steps 8,9,10,11,12
represents an approximation for the following:
$x^{k+1}=A x^{k}$.  So that a Krylov subspace is being generated.
To see this 
we define the overall 
nonlinear operator encompassing the $M$ time steps as:
\begin{eqnarray}
...
$N^{(M)}$ is the ``M'' time step discrete solution operator.
\par\noindent
So that, in this compact notation, step (10) is,
\begin{eqnarray}
	\delta S^{(k_{DMD}+1)}=
	N^{(M)}(S^{steady,(0)}+
	\delta S^{(k_{DMD})})-
...
Let,
\begin{eqnarray}
	A\equiv \frac{\partial N^{(M)}(S^{steady,(0)})}{\partial S}
\end{eqnarray}
Then using Taylor's theorem,
\begin{eqnarray}
	\delta S^{(k_{DMD}+1)}\approx 
	A \delta S^{(k_{DMD})}\approx
	A^{2} \delta S^{(k_{DMD}-1)}\approx\ldots\approx
	A^{k_{DMD}+1} \delta S^{(0)}
\end{eqnarray}
Ranjan et al\cite{RANJAN2020109076} (paper now included in this repository) 
recommends to 
use the DMD method by Schmid\cite{schmid2010dynamic} (this paper also
now included in this repository).

Another paper of interest is ``Constrained Dynamic Mode
Decomposition''; see \cite{9904965}.  Also this paper is now included in
this repository.

\subsection{Algorithm for approximating eigenvalues and eigenfunction from
  a Krylov subspace}

In this section only, 
we let $M$ be the dimension of eigenvectors for which we
seek and $S$ an $N-1\times N-1$ matrix whose eigenvalues approximate that of
$A$.  $A$ is an $M\times M$ matrix.
\par\noindent
%Stefan problem
%Peng Diao thesis (1D Stefan problem for exploring time parallel methods)
In \cite{schmid2010dynamic}, they first assume that there is a linear mapping
$A$ that connects the flow field $\vec{v}_{i}$ to the subsequent flow field
$\vec{v}_{i+1}$, in other words,
\begin{eqnarray}
\vec{v}_{i+1}=A\vec{v}_{i}.
\end{eqnarray}
It is assumed that $A$ is approximately independent of time over a given
full sampling interval $0\le t\le (N-1)\Delta t$.  A Krylov sequence can
be generated, given our assumptions, as follows:
\begin{eqnarray}
V_{1}^{N}=\left\{ v_{1},Av_{1},A^{2}v_{1},\ldots,A^{N-1}v_{1} \right\}.
\end{eqnarray}
The Krylov subspace $V_{1}^{N}$ has $N$ vectors; each vector $M\times 1$.
This implies that $V_{1}^{N}$ can be represented as a $M\times N$ matrix.
\par\noindent
It is assumed that $N<<M$.
\par\noindent
The strategy is to pick $N$ large enough so that the vectors that make up
$V_{1}^{N}$ are as close to linearly dependent as possible.  Then one has,
\begin{eqnarray}
\vec{v}_{N}=a_{1}\vec{v}_{1}+a_{2}\vec{v}_{2}+\ldots+a_{N-1}\vec{v}_{N-1}+
  \vec{r}
\end{eqnarray}
In matrix form (see (2.5) of \cite{schmid2010dynamic}), one has
\begin{eqnarray}
\vec{v}_{N}=V_{1}^{N-1}\vec{a}+\vec{r}. \label{aequation}
\end{eqnarray}
$\vec{a}$ is defined as
\begin{eqnarray}
\vec{a}^{T}=(a_{1},a_{2},\ldots,a_{N-1})
\end{eqnarray}
Next, in \cite{schmid2010dynamic} equation (2.6), one has
\begin{eqnarray}
\lefteqn{ AV_{1}^{N-1}=V_{2}^{N}= } \\
& & \left\{\vec{v}_{2},\vec{v}_{3},\vec{v}_{4},\ldots,V_{1}^{N-1}\vec{a}
    \right\}+\vec{r}\vec{e}_{N-1}^{T}=  \nonumber \\
& & V_{1}^{N-1}S+\vec{r}\vec{e}_{N-1}^{T}
\end{eqnarray}
where (\cite{schmid2010dynamic} equation (2.8)),
\begin{eqnarray}
S=\left( \begin{array}{ccccc}
  ...
  1 & 0        &         &   & a_{2} \\
    ...
    &          & 1       & 0 & a_{N-2} \\
    &          &         & 1 & a_{N-1} 
\end{array} \right) \label{Smatrix}
\end{eqnarray}
To summarize, one has,
\begin{eqnarray}
AV_{1}^{N-1}=V_{1}^{N-1}S+\vec{r}\vec{e}^{T}_{N-1}. \label{AandS}
\end{eqnarray}
$S$ is an $N-1\times N-1$ matrix.
\par\noindent
Suppose $\vec{r}=0$, then (from (\ref{aequation}))
\begin{eqnarray}
\vec{v}_{N}=V_{1}^{N-1}\vec{a}
\end{eqnarray}
Let,
\begin{eqnarray}
U\Sigma W^{\ast}=V_{1}^{N-1} \hspace{20pt}
\mbox{(SVD sorted from highest to lowest singular values)}
\end{eqnarray}
\begin{eqnarray}
\vec{v}_{N}=U\Sigma W^{\ast}\vec{a} \label{avrelation} 
\end{eqnarray}
From (\ref{avrelation}):
\begin{eqnarray}
	\vec{a}=W\Sigma^{\dagger}U^{\ast}\vec{v}_{N} \label{not_efficient}
\end{eqnarray}
$V_{1}^{N-1}$ is a $M\times N-1$ matrix.
$U$ is an $M\times M$ matrix.  $\Sigma$ is a $M\times N-1$ matrix. $W$ is
a $N-1\times N-1$ matrix.
\par\noindent
IMPORTANT REMARK:
There should be a way, using the ``Implicitly restarted
Arnoldi method''\cite{lehoucq2001implicitly}, in order to find the
top eigenvalues and eigenvectors of $V_{1}^{N-1}$
using just matrix vector multiplies where the matrix is 
$V_{1}^{N-1}$.
\par\noindent
ANOTHER IMPORTANT REMARK:
one can solve
\begin{eqnarray}
	\vec{v}_{N}=V_{1}^{N-1}\vec{a} \label{a_equation}
\end{eqnarray}
for $\vec{a}$ by framing the problem as a least squares  minimization 
problem.
\par\noindent
So now, given that $\vec{a}$ is determined, we can find $S$ using
(\ref{Smatrix}).  Then, using (\ref{AandS}) and ignoring $\vec{r}$,  is to
observe that,
\begin{eqnarray}
AU\Sigma W^{\ast}=U\Sigma W^{\ast} S
\end{eqnarray}
\begin{eqnarray}
U^{\ast}AU\Sigma W^{\ast}=\Sigma W^{\ast} S
\end{eqnarray}
\begin{eqnarray}
U^{\ast}AU\Sigma =\Sigma W^{\ast} S W
\end{eqnarray}
\begin{eqnarray}
U^{\ast}AU =\Sigma W^{\ast} S W \Sigma^{\dagger} \label{similar}
\end{eqnarray}
The matrix on the right of (\ref{similar}) is an $M\times M$ matrix which
is large! But the dimensions of $S$ is $N-1\times N-1$.  So we find the
eigenvalues and eigenvectors of $S$:
\begin{eqnarray}
S=X\Gamma X^{-1}  \hspace{20pt}
\mbox{(sorted from highest to lowest magnitude eigenvalues)}
\label{Sspectrum}
\end{eqnarray}
Using (\ref{Sspectrum}) and (\ref{similar}), one has
\begin{eqnarray}
A \approx P\Gamma P^{\dagger} \hspace{20pt} P=U\Sigma W^{\ast}X=V_{1}^{N-1}X
\end{eqnarray}
$P$ is an $M\times N-1$ matrix.

So now using an ``approximate'' similarity transpose, we have that
the first $N-1$ eigenvectors of $A$ are approximately the columns of 
$P$ and the dominant eigenvalues of $A$ are the diagonal entries of
$\Gamma$.

\subsection{Equivalency of the above analysis to
  the least squares approach and a new generalization}

In re-considering equations (\ref{avrelation}) through (\ref{a_equation}), 
one can cast the problem as a least squares problem without ever explicitly
finding the $M\times M$ matrix $U$.  In order to simplify the notation, define:
\begin{eqnarray}
	B \equiv V_{1}^{N-1}.
\end{eqnarray}
The least squares problem is:
\begin{eqnarray}
	\vec{a}=\mbox{argmin} J(\vec{a})=
	\mbox{argmin}_{\vec{a}} \sum_{i=1}^{M}\left(
	(\sum_{j=1}^{N-1}B_{ij}a_{j})-v_{Ni}\right)^{2}w_{i}
\end{eqnarray}
The system of equations corresponding to,
\begin{eqnarray}
	\frac{\partial J}{\partial a_{k}}=0 \hspace{10pt} k=1,\ldots,N-1,
\end{eqnarray}
is,
\begin{eqnarray}
	B^{T}WB\vec{a}=B^{T}W\vec{v}_{N}. \label{lowdim}
\end{eqnarray}
$W$ is a $M\times M$ diagonal matrix with the following components:
\begin{eqnarray}
	...
	\right.
\end{eqnarray}
The components of $B^{T}WB$ and $B^{T}W\vec{v}_{N}$ are weighted
inner products of the Krylov subspace vectors amongst 
themselves:
\begin{eqnarray}
	(B^{T}WB)_{ij}=(\vec{v}_{i},\vec{v}_{j})_{\vec{w}} \hspace{10pt} 
	i,j=1,\ldots,N-1
\end{eqnarray}
\begin{eqnarray}
	(B^{T}W\vec{v}_{N})_{i}=(\vec{v}_{i},\vec{v}_{N})_{\vec{w}} 
	\hspace{10pt} i=1,\ldots,N-1
\end{eqnarray}
Remarks:
\begin{enumerate}
	\item Solving (\ref{lowdim}) for $\vec{a}$,
		by finding the pseudo inverse (Moore-Penrose inverse)
		of
		$B^{T}WB$, is equivalent to (\ref{not_efficient})
		when $W$ is the identity.
	\item The weights should be chosen according to the positioning
		of the zero
		level set of ``$\phi^{N}$''. For interfacial
		flows controlled by the interface, the weights corresponding
		to the level set function variable should dominate over
		the weights corresponding to the velocity variables.
\end{enumerate}
