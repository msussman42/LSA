...
\begin{description}
	...
	\item[8.] Add the normalized perturbations to the base 
		...
		\sigma\Delta x(\delta \phi^{(k_{power})})^{norm}
                \end{eqnarray}
		Important note: we can write the overall perturbation in the
		following form:
                \begin{eqnarray}
			(\delta S^{(k_{power})})^{norm}=\left(
			\begin{array}{ccc}
				\gamma_{u}^{(k_{power})}\mathbf{I} & 0 & 0 \\
				0 & \gamma_{T}^{(k_{power})}\mathbf{I} & 0 \\
				0 & 0 & \gamma_{\phi}^{(k_{power})}\mathbf{I}
			\end{array} \right)
			\delta S^{(k_{power})}\equiv \\
			\mbox{diag}(\vec{\gamma}^{(k_{power})})
			\delta S^{(k_{power})}
                \end{eqnarray}
...
\begin{eqnarray}
	...
	\mbox{diag}(\vec{\gamma}^{(k_{power})})
	A 
	\mbox{diag}(\vec{\gamma}^{(k_{power}-1)})
	\delta S^{(k_{power}-1)}= \ldots \\
\end{eqnarray}
