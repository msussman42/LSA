...
\begin{description}
	...
	\item[2.] Using $S^{steady,(0)}$ as initial conditions, we solve 
		...
		We call the resulting new state $S^{steady,(1)}$.
	\item[3.] construct a default velocity perturbation and
		normalize the perturbation so that the maximum value is one.
		Multiply the perturbation by $\epsilon_{V}$ and add it to
		the base state 
		$S^{steady,(0)}$.  
	\item[4.] construct a default temperature perturbation and
		normalize the perturbation so that the maximum value is one.
		Multiply the perturbation by $\epsilon_{T}$ and add it to
		the base state
		$S^{steady,(0)}$.  
	\item[5.] construct a default levelset  perturbation which will be
