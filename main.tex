...
In this section only, we let $M$ be the dimension of eigenvectors for which we
seek.  $A$ is an $M\times M$ matrix.
\par\noindent
%Stefan problem
...
$A$ connects the flow field $\vec{v}_{i}$ to the subsequent flow field
...
\begin{eqnarray}
\vec{v}_{N}=V_{1}^{N-1}\vec{a}+\vec{r}.
\end{eqnarray}
$\vec{a}$ is defined as
\begin{eqnarray}
...
\begin{eqnarray}
AV_{1}^{N-1}=V_{1}^{N-1}S+\vec{r}\vec{e}^{T}_{N-1}.
\end{eqnarray}
$S$ is an $N-1\times N-1$ matrix.
\par\noindent
Suppose $\vec{r}=0$, then
\begin{eqnarray}
\vec{v}_{N}=V_{1}^{N-1}\vec{a}
\end{eqnarray}
Let,
\begin{eqnarray}
U\Sigma W^{\ast}=V_{1}^{N-1}
\end{eqnarray}
\begin{eqnarray}
\vec{v}_{N}=U\Sigma W^{\ast}\vec{a}
\end{eqnarray}
\begin{eqnarray}
...
\par\noindent
So now, given that $\vec{a}$ is determined, is to
observe that (ignoring $\vec{r}$),
\begin{eqnarray}
...
\end{eqnarray}
\begin{eqnarray}
U^{\ast}AU =\Sigma W^{\ast} S W \Sigma^{\dagger}
\end{eqnarray}
One can further find the singular value decomposition of $S$,
\begin{eqnarray}
U_{2}\Sigma_{2} W_{2}^{\ast}=S
\end{eqnarray}
$U_{2}$ is an $N-1\times N-1$ matrix.  $\Sigma_{2}$ is a diagonal 
$N-1\times N-1$ matrix.  $W_{2}$ is $N-1\times N-1$.
\par\noindent
So now one has,
\begin{eqnarray}
AX_{1}=X_{2}\Lambda
...
and
\begin{eqnarray}
X_{1}=U\Sigma W^{\ast}W_{2} \\
X_{2}=U\Sigma W^{\ast}U_{2}
\end{eqnarray}
$\Lambda$ is a diagonal $N-1\times N-1$ matrix.  $X_{1}$ is a $M\times N-1$
matrix.  $X_{2}$ is a $M\times N-1$ matrix.
