...
\begin{description}
	...
	\item[2.] Using $S^{steady,(0)}$ as initial conditions, we solve 
		...
		$S_{alt}^{steady,(0)}$.
		$S_{alt}^{steady,(0)}$ is the state that one obtains if the
	...
	\item[5.] normalize the velocity perturbation 
		so that the maximum value is one.  The normalized velocity
		perturbation is called:
                \begin{eqnarray}
                (\delta u^{(k_{power})})^{norm}
                \end{eqnarray}
	\item[6.] normalize the temperature perturbation 
		so that the maximum value is one.  The normalized temperature
		perturbation is called:
                \begin{eqnarray}
                (\delta T^{(k_{power})})^{norm}
                \end{eqnarray}
	\item[7.] Make sure the input levelset  perturbation is
		...
		\begin{eqnarray*}
			...
			|\delta\phi^{(k_{power})}|
		\end{eqnarray*}
		Multiply $\delta\phi^{(k_{power})}$ by $1/\beta$; the 
		normalized
		level set perturbation is
                \begin{eqnarray}
		(\delta\phi^{(k_{power})})^{norm}=\frac{1}{\beta}
		\delta\phi^{(k_{power})} 
		\end{eqnarray}

	\item[8.] Add the normalized perturbations to the base 
		state $S^{steady,(0)}$ (velocity, temperature, and 
		level set function):
                \begin{eqnarray}
                u^{perturb,(0)}=u^{steady,(0)}+
                   \epsilon_{u}(\delta u^{(k_{power})})^{norm}
                \end{eqnarray}
                \begin{eqnarray}
                T^{perturb,(0)}=T^{steady,(0)}+
                   \epsilon_{T}(\delta T^{(k_{power})})^{norm}
                \end{eqnarray}
                \begin{eqnarray}
                \phi^{perturb,(0)}=\phi^{steady,(0)}+
		\sigma\Delta x(\delta \phi^{(k_{power})})^{norm}
                \end{eqnarray}
        ...
        \item[10.] Update the perturbation prediction:
                \begin{eqnarray}
                \delta u^{(k_{power}+1)}=u^{perturb,M}-u_{alt}^{(0)}
                \end{eqnarray}
                \begin{eqnarray}
                T^{(k_{power}+1)}=T^{perturb,M}-T_{alt}^{(0)}
                \end{eqnarray}
                \begin{eqnarray}
                \phi^{(k_{power}+1)}=\phi^{perturb,M}-\phi_{alt}^{(0)}
                \end{eqnarray}
        \item[11.] $k_{power}=k_{power}+1$
        \item[12.] Go back to step 5.

\end{description}
Important Remark: Taken in succession, the steps 8,9,10,11,5,6,7
represents an approximation for the power method:
$x^{k+1}=A x^{k}$.  To see this we define the overall 
...
\begin{eqnarray}
	\delta S^{(k_{power}+1)}=
	N^{(M)}(S^{steady,(0)}+\epsilon D \delta S^{(k_{power})})-
	N^{(M)}(S^{steady,(0)})
\end{eqnarray}
$D$ is a 3x3 diagonal matrix.  Let,
\begin{eqnarray}
	A\equiv \frac{\partial N^{(M)}(S^{steady,(0)})}{\partial S}
\end{eqnarray}
Then using Taylor's theorem,
\begin{eqnarray}
	\delta S^{(k_{power}+1)}\approx 
	\epsilon A D \delta S^{(k_{power})}
\end{eqnarray}
Then after step (10), one does the normalization steps, 5,6,7, so that one has
\begin{eqnarray}
	\delta S^{(k_{power}+1)}\approx 
	\alpha (D^{new})^{-1} A D \delta S^{(k_{power})}
\end{eqnarray}
If we revisit these equations component by component, one has:
\begin{eqnarray}
	\delta u^{(k_{power}+1)}=
	u^{(M)}(S^{steady,(0)}+
	\epsilon D\delta S^{(k_{power})})-
	u^{(M)}(S^{steady,(0)})
\end{eqnarray}
Let,
\begin{eqnarray}
	A_{u}\equiv \frac{\partial u^{(M)}(S^{steady,(0)})}{\partial S}
\end{eqnarray}
\begin{eqnarray}
	\delta u^{(k_{power}+1)}=\epsilon A_{u}D\delta S^{(k_{power})}
\end{eqnarray}
As is clear here, the power method will not work! When different variables are
normalized differently, one no longer has the $p$th iterate proportional
to $A^{p}x$ for some initial guess $x$.  Also, the maximum eigenvalue in
magnitude might be negative.  It is recommended by 
Ranjan et al\cite{RANJAN2020109076} (paper now included in this repository) to 
